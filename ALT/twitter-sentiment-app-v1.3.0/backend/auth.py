...
def create_access_token(data, expires_delta:Optional[timedelta]=None):
    to_encode=data.copy(); expire=datetime.utcnow()+ (expires_delta or timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)); to_encode.update({'exp':expire});
...
def get_current_user(token:Optional[str]=None, bearer:str=Depends(oauth2_scheme)):
    actual=token or bearer; payload=decode_token(actual); username: str=payload.get('sub')
    if not username: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid token payload')
