...
def ensure_user(username,password,pro=False):
    with SessionLocal() as db:
        if get_user_by_username(db, username): print(f'User {username} exists'); return
        hashed=get_password_hash(password); create_user(db, username, hashed, pro=pro); print(f'Created {username} (pro={pro})')
