...
app=FastAPI(title='Twitter Sentiment API',version='1.4.0')
...
async def signup(payload:SignupModel):
    with SessionLocal() as db:
        ...
        tok=create_access_token({'sub':payload.username}); return Token(access_token=tok,token_type='bearer')
...
async def login(form_data:OAuth2PasswordRequestForm=Depends()):
    with SessionLocal() as db:
        ...
        tok=create_access_token({'sub':user.username}); return Token(access_token=tok,token_type='bearer')
