...
from schemas import Token, SignupModel, SentimentItem, UserOut
from sentiment_logic import get_sentiment_list, realtime_stream
app = FastAPI(title="Twitter Sentiment API", version="1.0.7a")
...
async def signup(payload: SignupModel):
    with SessionLocal() as db:
        if get_user_by_username(db, payload.username): raise HTTPException(status_code=400, detail="Username already exists")
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    with SessionLocal() as db:
        ...
        if not user or not verify_password(form_data.password, user.password_hash): raise HTTPException(status_code=400, detail="Incorrect username or password")
...
async def free_daily(tickers: Optional[str] = None, limit: int = 50, sort: Optional[str] = "interest_score"):
    return get_sentiment_list(live=False, tickers=tickers.split(",") if tickers else None, limit=limit, sort=sort)
...
async def pro_snapshot(window: Optional[str] = "5m", user: Dict[str, Any] = Depends(get_current_user)):
    ...
    return get_sentiment_list(live=True, limit=50, sort="interest_score")
@app.websocket("/ws/realtime")
async def ws_realtime(ws: WebSocket):
    token = ws.query_params.get("token")
    try:
        user = get_current_user(token=token)
        if not user.get("pro"): await ws.close(code=4403); return
    except Exception: await ws.close(code=4401); return
    await ws.accept()
    try:
        async for payload in realtime_stream(): await ws.send_json(payload)
