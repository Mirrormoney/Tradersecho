...
app = FastAPI(title="Twitter Sentiment API", version="1.1.0")
...
async def signup(payload: SignupModel):
    with SessionLocal() as db:
        ...
        create_user(db, payload.username, hashed, pro=False)
...
async def pro_snapshot(window: Optional[str] = "5m", user: Dict[str, Any] = Depends(get_current_user)):
    if not user.get("pro"):
        raise HTTPException(status_code=403, detail="Upgrade to Pro to access this endpoint")
...
async def ws_realtime(ws: WebSocket):
    ...
    await ws.accept()
    try:
        async for payload in realtime_stream():
