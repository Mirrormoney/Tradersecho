...
def render(report, name='there', origin='https://tradersecho.com', preview=False):
    ...
    def change(row):
        return f"{row['change']:+g}% vs prior {report.get('period_label','day')}" if row.get('change') is not None else 'Comparison unavailable · history building'
    def stock(row,index):
