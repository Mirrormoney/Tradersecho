...
def render(report, name='there', origin='https://tradersecho.com', preview=False):
    ...
    rows=report.get('rows',[])[:5]
    watched=report.get('watchlist',[])[:5]
    posts=report.get('posts',[])[:3]
    ...
    for p in posts:
        ...
        voice_html+=f'''<div style="padding:18px;background:#edf3eb;margin:12px 0;border-left:3px solid #84a35b"><strong style="font-size:13px;color:#123e33">@{e(p['author'])}</strong><p style="font-size:14px;line-height:1.7;color:#344d40;white-space:pre-line">{e(excerpt)}</p><a href="https://x.com/i/web/status/{e(p['id'])}" style="font-size:12px;color:#285e46">Read the original on X →</a></div>'''
    if voice_html:voice_html='<h2 style="font-size:23px;color:#123e33;margin:30px 0 8px">Voices worth a closer look.</h2><p style="color:#64766c;font-size:13px">Collected excerpts from your followed and shared accounts. Not endorsements.</p>'+voice_html
