...
def render(report, name='there', origin='https://tradersecho.com', preview=False):
    ...
    for p in posts:
        ...
        voice_html+=f'''<div style="padding:18px;background:#edf3eb;margin:12px 0;border-left:3px solid #84a35b"><strong style="font-size:13px;color:#123e33">@{e(p['author'])}</strong><span style="font-size:12px;color:#64766c"> · {e(' / '.join('$'+t for t in (p.get('tickers') or '').split(',') if t))}</span><p style="font-size:14px;line-height:1.7;color:#344d40;white-space:pre-line">{e(excerpt)}</p><a href="https://x.com/i/web/status/{e(p['id'])}" style="font-size:12px;color:#285e46">Read the original on X →</a></div>'''
    if voice_html:voice_html='<h2 style="font-size:23px;color:#123e33;margin:30px 0 8px">Behind the top three.</h2><p style="color:#64766c;font-size:13px">Relevant collected posts from shared and followed accounts, selected by engagement, with one per author. Not endorsements.</p>'+voice_html
    if not voice_html:voice_html='<h2 style="font-size:23px;color:#123e33">Behind the top three.</h2><p style="font-size:13px;color:#64766c">No qualifying posts from shared or followed accounts in this report window. Open a stock to explore its current discussion.</p>'
