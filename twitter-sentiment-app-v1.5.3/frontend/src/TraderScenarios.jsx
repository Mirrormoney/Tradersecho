...
export function TraderScenarios(){
 ...
 return <section className={'trader-scenarios '+(paused?'is-paused':'')} aria-labelledby="scenarios-title">
  <div className="section-head"><div><span className="eyebrow">DIFFERENT ROUTINES. A SHARED CURIOSITY.</span><h2 id="scenarios-title">Make room for a better research habit.</h2><p className="muted">Fictional trader perspectives: illustrative names and quotes, not customer reviews.</p></div><button className="button" aria-pressed={paused} onClick={()=>setPaused(!paused)}>{paused?'Play cards':'Pause cards'}</button></div>
  <div className="scenario-window"><div className="scenario-track">{[0,1].map(copy=><div className="scenario-group" key={copy} aria-hidden={copy===1?true:undefined}>{scenarios.map(([title,body,n,tag])=><article className="scenario-card" key={title}><div className="scenario-card-top"><span className="scenario-mark">↗</span><span>{tag}</span></div><p>{body}</p><div className="scenario-person"><span className="avatar">{n}</span><div><h3>{title}</h3><small>Fictional persona · illustrative quote</small></div></div></article>)}</div>)}</div></div>
 </section>
}
