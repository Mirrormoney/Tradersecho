...
export function Voices({admin=false,premium=true,onChange,onPlans}){
 ...
 return <section className="panel">
  ...
  <p className="muted">{admin?'Manage the curated accounts shared with all signed-in members. There is no account-count limit on this admin list. The registry combines these with eligible members’ personal follows.':'The curated list is available to every signed-in member. Premium adds five personal accounts.'} Each X account is collected once for everyone. Opening this page never buys more posts.</p>
  {error&&<p className="error" role="alert">{error}</p>}{notice&&<p className="positive" role="status">{notice}</p>}
  {!data&&<p>Loading voices…</p>}
  {curated?.map(h=><div className="handle" key={h.handle}><div><strong>@{h.handle}</strong><p>{h.note||'Admin-selected · shared with all members'}</p><small>{h.checked_at?'Last checked '+new Date(h.checked_at*1000).toLocaleString()+' · ':''}{h.collection_message}{h.retry_at?' · Resets '+new Date(h.retry_at*1000).toLocaleString():''}{h.truncated?' · Capped sample':''}</small></div>{admin&&<button className="button" disabled={busy} onClick={()=>change(()=>api('/admin/voices/'+h.handle,'DELETE'),'Removed from curated voices. Personal follows and collected posts are retained.')}>Remove curated @{h.handle}</button>}</div>)}
